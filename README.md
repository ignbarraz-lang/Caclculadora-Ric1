[index.html](https://github.com/user-attachments/files/25415291/index.html)
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
...
<div class="container">
    <div class="card">
        ...
        <label>Sección (mm²):</label> <input type="number" id="seccion" value="6">
        <button class="btn-calc" onclick="calcularCaida()">Calcular Caída</button>
        <div id="resCaida"></div>
    </div>

    <div class="card">
        <h2>🕳️ Ductos EMT (RIC 07)</h2>
        <label>Cable RVK:</label>
        <select id="tipoCable">
            <option value="52.8">6mm² (8.2mm Ø)</option>
            <option value="81.7">10mm² (10.2mm Ø)</option>
            <option value="109.4">16mm² (11.8mm Ø)</option>
        </select>
        ...
        <button class="btn-ducto" onclick="calcularDuctos()">Analizar Ocupación</button>
        <div id="resDucto"></div>
    </div>
</div>
...
function calcularCaida() {
    ...
    document.getElementById('resCaida').innerHTML = `<div class="result ${clase}">Pérdida: ${dv.toFixed(2)}V (${porc.toFixed(2)}%)<br>${porc <= 3 ? 'DENTRO DE NORMA ✅' : 'FUERA DE NORMA ❌'}</div>`;
}
...
function calcularDuctos() {
    ...
    let html = `<div class="result area-info">Área Requerida: ${areaTotal.toFixed(1)} mm² | Ocupación: ${factor*100}%</div>`;
    let yaRec = false;
    ductos.forEach(d => {
        let cap = ((Math.PI * Math.pow(d.d, 2)) / 4) * factor;
        if (cap >= areaTotal) {
            if (!yaRec) { html += `<div class="result recomendado">${d.n}: RECOMENDADO ✅</div>`; yaRec = True; }
            else { html += `<div class="result excesivo">${d.n}: EXCESIVO 🔵</div>`; }
        } else { html += `<div class="result falla">${d.n}: SOBRECARGADO ❌<br><small>Capacidad máx: ${cap.toFixed(1)}mm²</small></div>`; }
    });
